...
from django.contrib.auth.backends import (
    BaseBackend as BaseBackend_,
)

UserModel = get_user_model()


class BaseBackend(BaseBackend_):
...
class ModelBackend(BaseBackend):
    ...
    def _get_role_permissions(self, user_obj):
        user_role_field = get_user_model()._meta.get_field('role')
        user_role_query = 'role__%s' % user_role_field.related_query_name()
        return Permission.objects.filter(**{user_role_query: user_obj})
